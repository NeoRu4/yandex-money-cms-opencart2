...
class ControllerPaymentYamodule extends Controller
{
    ...
    public function index()
    {
        ...
        if ($this->isB2bSberbank() && $this->isWrongNumberVatTypes($order_info)) {
            $data['b2b_sberbank_error'] = $this->language->get('b2b_sberbank_vat_error');
        }
        ...
        if ($this->isB2bSberbank()) {

            $data['is_b2b_sberbank']              = 1;
            $data['b2b_sberbank_payment_purpose'] = $this->createDescription($order_info);
            $vatTypeAndRate                       = $this->calculateVatTypeAndRate($order_info);
            $data['b2b_sberbank_vatType']         = $vatTypeAndRate['vatType'];
            if ($vatTypeAndRate['vatType'] === B2bSberbank::VAT_TYPE_CALCULATED) {
                $data['b2b_sberbank_vatRate'] = $vatTypeAndRate['vatRate'];
                $vatSumAmount                 = $this->currency->format($order_info['total'],
                        $order_info['currency_code'], $order_info['currency_value'],
                        false) * $vatTypeAndRate['vatRate'] / 100;
                $data['b2b_sberbank_vatSum']  = round($vatSumAmount, 2);
            }
        } else {
            $data['is_b2b_sberbank'] = 0;
        }
        ...
        if (file_exists(DIR_TEMPLATE.$this->config->get('config_template').'/template/'.$for23.'payment/yamodule'.$end_tpl)) {
            return $this->load->view($this->config->get('config_template').'/template/'.$for23.'payment/yamodule'.$end_tpl,
                $data);
        } else {
        ...
        if ($this->session->data['payment_method']['code'] == 'yamodule') {
            ...
            if (isset($this->request->get['payMethod']) && $this->request->get['payMethod'] == 'fast_pay') {
                $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],
                    $this->config->get('ya_fast_pay_os'), '', true);
            } else {
                if ($this->config->get('ya_kassa_create_order') == '1') {
                    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],
                        $this->config->get('config_order_status_id'), '', true);
                }
            }

            if ($this->config->get('ya_kassa_cart_reset') == '1') {
                $this->cart->clear();
            }
        }
    }
    ...
    public function yaredirect()
    {
        if (isset($_POST['payment-type']) && !empty($_POST['payment-type'])) {
            if ($_POST['payment-type'] == 'wallet') {
                ...
                $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
                // $this->d($total);
                $limit    = number_format($this->currency->convert($this->currency->format($order_info['total'],
                    $order_info['currency_code'], $order_info['currency_value'], false), $order_info['currency_code'],
                    'RUB'), 2, '.', '');
                $scope    = array(
                ...
                $for23    = (version_compare(VERSION, "2.3.0", '>=')) ? "extension/" : "";
                $auth_url = $this->buildObtainTokenUrl($this->config->get('ya_p2p_idapp'),
                    $this->url->link($for23.'payment/yamodule/insidewallet', '', 'SSL'), $scope);
    ...
    public function successcard()
    {
        if ($this->session->data['cps_context_id'] == $_GET['cps_context_id']) {
            $this->load->model('checkout/order');
            $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],
                $this->config->get('ya_p2p_os'), '', true);
    ...
    public function insidecard()
    {
        ...
        if ($instance->status == 'success') {
            ...
            $this->load->model('checkout/order');
            $order_info      = $this->model_checkout_order->getOrder($this->session->data['order_id']);
            $total           = number_format($this->currency->convert($this->currency->format($order_info['total'],
                $order_info['currency_code'], $order_info['currency_value'], false), $order_info['currency_code'],
                'RUB'), 2, '.', '');
    ...
    public function insidewallet()
    {
        ...
        if (empty($code)) {
            ...
            $response = $this->getAccessToken($this->config->get('ya_p2p_idapp'), $code,
                $this->url->link($for23.'payment/yamodule/yaredirect', '', 'SSL'), $this->config->get('ya_p2p_pw'));
            $error    = '';
            $token    = '';
    ...
    public function callback()
    {
        ...
        $order_id = isset($data[self::ORDERNUMBER]) ? (int)substr($data[self::ORDERNUMBER], strlen(self::PREFIX_DEBUG),
            strlen($data[self::ORDERNUMBER])) : 0;
        if ($this->config->get('ya_kassa_active') && isset($data['action']) && !empty($data['action'])) {
            $this->log_save('callback:  orderid='.$order_id);
            if ($this->{"model_".str_replace("/", "_", $for23)."yamodel_yamoney"}->checkSign($data, $password, $shopid,
                true)
            ) {
                $this->load->model('checkout/order');
                $order_info = $this->model_checkout_order->getOrder($order_id);
                $sum        = number_format($this->currency->convert($this->currency->format($order_info['total'],
                    $order_info['currency_code'], $order_info['currency_value'], false), $order_info['currency_code'],
                    'RUB'), 2);
                ...
                if (number_format($data['orderSumAmount'], 2) == $sum) {
                    ...
                    $this->{"model_".str_replace("/", "_", $for23)."yamodel_yamoney"}->sendCode($data, $shopid, '100',
                        "Error total amount");
                }
            }
        } else {
     ...
     * @return bool
     */
    private function isB2bSberbank()
    {
        return $this instanceof ControllerPaymentYamoduleB2bSberbank;
    }
    ...
    {
        $descriptionTemplate = $this->config->get('ya_b2b_sberbank_description_template') ?: $this->language->get('b2b_sberbank_template_default');
    ...
    {
        $taxRates = $this->config->get('ya_b2b_sberbank_taxes');
        ...
        if ($vatType === B2bSberbank::VAT_TYPE_UNTAXED) {
            return array(
                'vatType' => $vatType,
            );
        }
        ...
        return array(
            'vatType' => B2bSberbank::VAT_TYPE_CALCULATED,
            'vatRate' => $vatType,
        );
    }
     ...
     * @throws Exception
     */
    private function isWrongNumberVatTypes($order)
    {

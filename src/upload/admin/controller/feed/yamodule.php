...
class ControllerFeedYamodule extends Controller
{
    ...
    public $fields_kassa = array(
        ...
        'ya_kassa_def_del_payment_mode',
        'ya_kassa_def_del_payment_subject',
    );
    ...
    public $paymentSubjectEnum = array(
    ...
    public function processSave()
    {
        ...
        switch ($this->request->post['type_data']) {
            case 'kassa':
                ...
                if (isset($this->request->post['ya_kassa_active']) && $this->request->post['ya_kassa_active'] == 1) {
                    $testUrl                               = $this->url->link($for23.'payment/yamodule/test',
            ...
            case 'metrika':
                ...
                $this->{'model_'.str_replace("/", "_", $for23).'yamodule_metrika'}->processCounter($yaMetrika_number,
                    $yaMetrika_token);
                break;
            case 'market':
        ...
        if (!$this->Sget('ya_kassa_active') && !$this->Sget('ya_p2p_active') && !$this->Sget('ya_fast_pay_active')) {
            ...
            $this->model_setting_setting->editSetting('yamoduleb2bsberbank_status',
                array('yamoduleb2bsberbank_status' => 0));
        } else {
            $this->model_setting_setting->editSetting('yamodule_status', array('yamodule_status' => 1));
            $this->model_setting_setting->editSetting('yamoduleb2bsberbank_status',
                array('yamoduleb2bsberbank_status' => 1));
        }
        if ($updater !== false) {
            foreach (array('kassa', 'p2p', 'metrika', 'market', 'pokupki', 'fast_pay') as $type) {
                $this->session->data[$type.'_status'][] = $this->success_alert($updater, 'warning');
            }
        }
    ...
    public function initForm($array)
    {
        ...
        $data['ya_metrika_callback']     = str_replace('http://', 'https://',
            $this->url->link($for23.'feed/yamodule/preparem', 'token='.$this->session->data['token'], true));
        ...
        $data['ya_pokupki_callback']     = str_replace('http://', 'https://',
            $this->url->link($for23.'feed/yamodule/preparep', 'token='.$this->session->data['token'], true));
    ...
    {
        return $this->gocurl('m',
            'grant_type=authorization_code&code='.$this->request->get['code'].'&client_id='.$this->config->get('ya_metrika_idapp').'&client_secret='.$this->config->get('ya_metrika_pw'));
    }

    public function preparep()
    {
        return $this->gocurl('p',
            'grant_type=authorization_code&code='.$this->request->get['code'].'&client_id='.$this->config->get('ya_pokupki_idapp').'&client_secret='.$this->config->get('ya_pokupki_pw'));
    }
    ...
    public function gocurl($type, $post)
    {
        ...
        if ($status == 200) {
            if (!empty($data->access_token)) {
                ...
                if ($type == 'm') {
                    $this->model_setting_setting->editSetting('ya_metrika_o2auth',
                        array('ya_metrika_o2auth' => $data->access_token));
                } elseif ($type == 'p') {
                    $this->model_setting_setting->editSetting('ya_pokupki_gtoken',
                        array('ya_pokupki_gtoken' => $data->access_token));
                }
                $this->response->redirect($this->url->link('extension/feed/yamodule',
                    'token='.$this->session->data['token'], 'SSL'));
            }
        }

        $this->response->redirect($this->url->link('extension/feed/yamodule',
            'err='.$data->error_description.'&token='.$this->session->data['token'], 'SSL'));
    }
    ...
    public function index()
    {
        ...
        $array_init              = array_merge($this->fields_p2p, $this->fields_kassa, $this->fields_metrika,
            $this->fields_market, $this->fields_pokupki);
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            $this->processSave();
            $this->session->data['success'] = $this->language->get('text_success');
            $this->response->redirect($this->url->link($for23.'feed/yamodule', 'token='.$this->session->data['token'],
                'SSL'));
        }
        ...
        $arLang = array(
            ...
            'kassa_text_dynamic',
            'kassa_text_help_url',
            'kassa_text_help_cburl',
            'kassa_text_status',
            'kassa_text_debug_help',
            'kassa_text_debug_dis',
            'kassa_text_debug_en',
            'kassa_text_debug',
            'kassa_text_adv_head',
            'kassa_text_paylogo_help',
            'kassa_paylogo_text',
            'kassa_text_cart_reset',
            'kassa_cart_reset_text',
            'kassa_text_create_order',
            'kassa_create_order_text',
            'kassa_text_sort_order',
            'kassa_text_pay_help',
            'kassa_text_paymode_help',
            'kassa_text_paymode_shop',
            'kassa_text_paymode_kassa',
            'kassa_text_paymode_label',
            'kassa_text_paymode_head',
            'kassa_text_pw',
            'kassa_text_scid',
            'kassa_text_sid',
            'kassa_text_get_setting',
            'kassa_text_lk_head',
            'kassa_sv',
            'kassa_text_inv',
            'kassa_text_invhelp',
            'kassa_text_inv_subj',
            'kassa_text_inv_subjhelp',
            'kassa_text_inv_logo',
            'kassa_text_inv_logohelp',
            'kassa_text_inv_text',
            'kassa_text_inv_texthelp',
            'kassa_text_inv_pattern',
            'p2p_text_connect',
            'p2p_text_enable',
            'p2p_text_url_help',
            'p2p_text_setting_head',
            'p2p_text_account',
            'p2p_text_appId',
            'p2p_text_appWord',
            'p2p_text_app_help',
            ...
            'fast_pay_os_text',
            'fast_pay_desc_text',
            'fast_pay_enable_label',
            'fast_pay_id_label',
            'fast_pay_purpose_label',
            'fast_pay_os_label',
            'fast_pay_os_text',
            'fast_pay_desc_text',
            'fast_pay_title',
            ...
            'b2b_sberbank_label',
            'b2b_sberbank_on',
            'b2b_sberbank_template_label',
            'b2b_sberbank_template_default',
            'b2b_sberbank_template_help',
            'b2b_sberbank_vat_default_label',
            'b2b_sberbank_vat_default_help',
            'b2b_sberbank_vat_label',
            'b2b_sberbank_vat_cms_label',
            'b2b_sberbank_vat_sbbol_label',
            'b2b_sberbank_vat_help',
            'kassa_default_payment_mode_label',
            'kassa_default_payment_subject_label',
            'kassa_default_delivery_payment_mode_label',
            'kassa_default_delivery_payment_subject_label',
        );
        foreach ($arLang as $lang_name) {
            $data[$lang_name] = $this->language->get($lang_name);
        }
        $data['mod_off'] = sprintf($this->language->get('mod_off'),
            $this->url->link('extension/payment', 'token='.$this->session->data['token'].'&extension=yamodule', true));
        ...
        $data['txt_mws_connect']   = sprintf($this->language->get('txt_mws_connect'),
            $this->url->link('tool/mws/csr', 'token='.$this->session->data['token'], 'SSL'));
        $data['success_mws_alert'] = sprintf($this->language->get('success_mws_alert'),
            $this->url->link('sale/order', 'token='.$this->session->data['token'], 'SSL'),
            $this->url->link('extension/feed/yamodule', 'token='.$this->session->data['token'], 'SSL'));
        ...
        $data['paymentModeEnum']               = $this->paymentModeEnum;
        $data['paymentSubjectEnum']            = $this->paymentSubjectEnum;
        $data['defaultPaymentMode']            = $this->Sget('ya_kassa_default_payment_mode');
        $data['defaultPaymentSubject']         = $this->Sget('ya_kassa_default_payment_subject');
        $data['defaultDeliveryPaymentMode']    = $this->Sget('ya_kassa_def_del_payment_mode');
        $data['defaultDeliveryPaymentSubject'] = $this->Sget('ya_kassa_def_del_payment_subject');
        ...
        if (isset($this->session->data['fast_pay_status']) && !empty($this->session->data['fast_pay_status'])) {
            if (is_array($data['fast_pay_status'])) {
                $data['fast_pay_status'] = array_merge($data['fast_pay_status'],
                    $this->session->data['fast_pay_status']);
            } else {
    ...
    public function changestatus()
    {
        ...
        if ($order_info['customer_id'] == $this->config->get('yandexbuy_customer')) {
            ...
            $json = $this->{'model_'.str_replace("/", "_", $for23).'yamodule_pokupki'}->sendOrder($order_id,
                $order_status_id, $comment);
        }
        //
    ...
    {
        $this->db->query("INSERT INTO ".DB_PREFIX."customer SET customer_group_id = '".(int)$data['customer_group_id']."', firstname = '".$this->db->escape($data['firstname'])."', lastname = '".$this->db->escape($data['lastname'])."', email = '".$this->db->escape($data['email'])."', telephone = '".$this->db->escape($data['telephone'])."', fax = '".$this->db->escape($data['fax'])."', custom_field = '".$this->db->escape(serialize($data['custom_field']))."', newsletter = '".(int)$data['newsletter']."', salt = '".$this->db->escape($salt = substr(md5(uniqid(rand(),
                true)), 0,
                9))."', password = '".$this->db->escape(sha1($salt.sha1($salt.sha1($data['password']))))."', status = '".(int)$data['status']."', safe = '".(int)$data['safe']."', date_added = NOW()");
                ...
                if ($folder == "admin") {
                    if (file_exists(DIR_APPLICATION.$file)) {
                        if (!unlink(DIR_APPLICATION.$file)) {
                            $error_log->write(DIR_APPLICATION.$file);
                        }
                    }
                } else {
                    if (file_exists(DIR_CATALOG.$file)) {
                        if (!unlink(DIR_CATALOG.$file)) {
                            $error_log->write(DIR_CATALOG.$file);
                        }
                    }
                }
            }
    ...
    private function installFastPaySettings()
    {
        ...
        $this->model_setting_setting->editSetting('ya_fast_pay_description',
            array('ya_fast_pay_description' => 'Номер заказа %order_id%. Оплата через Яндекс.Платежку'));
        $this->model_setting_setting->editSetting('ya_fast_pay_url',
            array('ya_fast_pay_url' => 'https://money.yandex.ru/fastpay/confirm'));
    }

    private function getPaymentModeEnum()
    {
    }

    private function getPaymentSubjectEnum()
    {
    }
}
